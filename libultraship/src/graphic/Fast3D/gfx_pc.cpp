...
static void gfx_flush(void) {
    if (buf_vbo_len > 0) {
		...
		if (buf_vbo_start > (32 * 1024 * 1024 / sizeof(float) - MAX_BUFFERED * (32 * 3))) {
			buf_vbo_start = 0;
			buf_vbo_len = 0;
		}
#else
...
#ifdef __vita__
		float out_v[4];
		float src[4] = {v->ob[0], v->ob[1], v->ob[2], 1.0f};
		#define x out_v[0]
		#define y out_v[1]
		#define z out_v[2]
		#define w out_v[3]
		matvec4_neon((float *)rsp.MP_matrix, src, out_v);
#else
...
#ifdef __vita__
	if (!buf_vbo)
		buf_vbo = (float *)vglAlloc(32 * 1024 * 1024, VGL_MEM_RAM);
#endif
