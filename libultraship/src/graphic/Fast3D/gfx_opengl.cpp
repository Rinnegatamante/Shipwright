...
#ifdef __vita__
#define RAND_NOISE "((random(float3(floor(gl_FragCoord.xy * noise_scale), float(frame_count))) + 1.0) / 2.0)"
#else
...
static struct ShaderProgram* gfx_opengl_create_and_load_new_shader(uint64_t shader_id0, uint32_t shader_id1) {
    ...
    if (f) {
		//printf("Loading cached shader %s\n", fname);
...
#ifdef __vita__
static void gfx_opengl_draw_triangles(float buf_vbo[], size_t buf_vbo_start, size_t buf_vbo_len, size_t buf_vbo_num_tris) {
#else
static void gfx_opengl_draw_triangles(float buf_vbo[], size_t buf_vbo_len, size_t buf_vbo_num_tris) {
	...
	vglBufferData(GL_ARRAY_BUFFER, buf_vbo + buf_vbo_start);
#else
