...
#ifdef __vita__
	{
		FILE *handle = fopen(szFileName, "rb");
		fseek(handle, 0, SEEK_END);
		pStream->Base.File.FileSize = ftell(handle);
		fseek(handle, 0, SEEK_SET);
		if (!otr_cache) {
			otr_cache = (uint8_t *)malloc(pStream->Base.File.FileSize);
			fread(otr_cache, 1, pStream->Base.File.FileSize, handle);
			fclose(handle);
			handle = fmemopen(otr_cache, pStream->Base.File.FileSize, "rb");
		}
		pStream->Base.File.hFile = (HANDLE)handle;
	}
#elif defined(STORMLIB_MAC) || defined(STORMLIB_LINUX)
    {
    ...
    DWORD dwBytesToRead)                    // Number of bytes to read from the file
{
...
#ifdef __vita__
	{
		...
		{
			fseek((FILE *)pStream->Base.File.hFile, ByteOffset, SEEK_SET);
			pStream->Base.File.FilePos = ByteOffset;
		}
        ...
        if(dwBytesToRead != 0)
        {
            bytes_read = fread(pvBuffer, 1, dwBytesToRead, (FILE *)pStream->Base.File.hFile);
            if(bytes_read == -1)
            {
                dwLastError = errno;
                return false;
